...
max_steps = 5000
epoch = 3
dataset_name = "mixdataset_75NLI_25conv"
dev1_dataset_name = "chineseNLI"
dev2_dataset_name = "Hallu2"

data_dir = "data/{dataset}".format(dataset=dataset_name)
data_dir_dev1 = "data/{dataset}".format(dataset=dev1_dataset_name)
data_dir_dev2 = "data/{dataset}".format(dataset=dev2_dataset_name)
...
dev_ds = feature_column.build_dataset('dev', data_dir=os.path.join(data_dir_dev1, 'dev'), shuffle=False, repeat=False, use_gz=False) \
                               .map(map_fn) \
                               .padded_batch(bsz, (0, 0, 0))

dev_ds_2 = feature_column.build_dataset('dev', data_dir=os.path.join(data_dir_dev2, 'dev'), shuffle=False, repeat=False, use_gz=False) \
                               .map(map_fn) \
                               .padded_batch(bsz, (0, 0, 0))
            ...
            for ids, sids, label in P.io.DataLoader(
                ...
                if step % 100 == 0:
                    ...
                    with P.no_grad():
                        ...
                        msg = '[step-%d]  acc %.5f '% (step, acc)+"dev1:{dev1}".format(dev1 = dev1_dataset_name)
                        logger.info(msg)
                        acc = []
                        for ids, sids, label in P.io.DataLoader(
                                dev_ds_2, places=P.CUDAPlace(0),
                                batch_size=None):
                            loss, logits = model(ids, sids, labels=label)
                            #print('\n'.join(map(str, logits.numpy().tolist())))
                            a = (logits.argmax(-1) == label)
                            acc.append(a.numpy())
                        acc = np.concatenate(acc).mean()
                        msg = '[step-%d]  acc %.5f '% (step, acc)+"dev2:{dev2}".format(dev2 = dev2_dataset_name)
                        logger.info(msg)
                        model.train()
                    if step % 1000 == 0:
